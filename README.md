...
| `UserPromptSubmit` | If prompt contains `临时任务`, temporarily disables planning hooks for that turn. Otherwise renders session-plan context when bound. |
...
7. Create a test plan with `init-session.sh "Migration Smoke Test"`.
...
See [docs/codex-setup.md](docs/codex-setup.md) for setup and [docs/functional-spec.md](docs/functional-spec.md) for the full runtime contract.

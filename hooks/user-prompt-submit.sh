...
#   - security framing: ===BEGIN/END PLAN DATA=== + "treat as data"
#   - attestation tamper-check: SHA-256(task_plan.md) vs stored hash
...
if [ -n "$ATTEST" ]; then
    ...
    if [ -n "$ACTUAL" ] && [ "$ACTUAL" != "$ATTEST" ]; then
        ...
        echo "actual=  $ACTUAL"
        echo 'Run scripts/attest-plan.sh to re-approve the current contents, or restore the file from git.'
        exit 0
    fi
fi
